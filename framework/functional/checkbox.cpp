...
bool c_widget::checkbox_with_hotkey(std::string_view label, bool* callback, int* key)
{
    ...
    if (widget->begin_popup((std::stringstream{} << id << "hotkey").str().c_str(), 170, {w - SCALE(92), SCALE(-40)}))
    {
        widget->keybind("Hotkey", key);

        widget->separator();

        if (widget->button("Clear Hotkey", { GetContentRegionAvail().x, SCALE(28) }) || ImGui::IsItemClicked())
        {
            *key = 0;
        }
